...
class AlarmManager extends utils.Adapter {
  ...
  getEMessageClient() {
    var _a, _b;
    const username = (_a = this.nativeConfig.apiUserId) == null ? void 0 : _a.trim();
    const password = (_b = this.nativeConfig.apiPassword) == null ? void 0 : _b.trim();
    if (!username || !password) {
      throw new Error("API User ID oder API Password fehlt");
    }
    return new import_emessageClient.EMessageClient({
      username,
      password
    });
  }
  serviceSupportsDirectResponse(service) {
    return service === "2wayS" || service === "eBos";
  }
  async setServiceConnection(isConnected) {
    await this.setStateAsync("info.connection", isConnected, true);
  }
  async setConnectionStatus(status, errorText = "") {
    await this.setStateAsync("info.connectionStatus", status, true);
    await this.setStateAsync("info.connectionError", errorText, true);
  }
  sanitizePagerFolderName(pagerId, fallbackIndex) {
    const safe = String(pagerId || "").trim().replace(/[^a-zA-Z0-9_]/g, "_").replace(/_+/g, "_").replace(/^_+|_+$/g, "");
    return safe ? `pager_${safe}` : `pager_${fallbackIndex + 1}`;
  }
  getTriggerKey(trigger, index) {
    var _a;
    const triggerId = String(trigger.id || "").trim();
    if (triggerId) {
      return triggerId;
    }
    return [((_a = trigger.stateId) == null ? void 0 : _a.trim()) || "", trigger.condition || "", trigger.compareValue || "", String(index)].join(
      "::"
    );
  }
  async ensureStaticObjects() {
    ...
    await this.setObjectNotExistsAsync("alarm.lastTriggerStateId", {
      type: "state",
      common: {
        name: "Letzte Ausl\xF6sung durch State",
        type: "string",
        role: "text",
        read: true,
        write: false,
    ...
    await this.setObjectNotExistsAsync("alarm.lastTriggeredPager", {
      type: "state",
      common: {
        name: "Zuletzt ausgel\xF6ster Pager",
        type: "string",
        role: "text",
        read: true,
        write: false,
  ...
  async ensurePagerObjects() {
    ...
    for (let i = 0; i < pagers.length; i++) {
      ...
      await this.setObjectNotExistsAsync(channelId, {
        type: "channel",
        common: {
          name: ((_a = pager.alias) == null ? void 0 : _a.trim()) || ((_b = pager.identifier) == null ? void 0 : _b.trim()) || `Pager ${i + 1}`
        },
        native: {
          pagerId: pager.id
        }
      ...
      await this.extendObjectAsync(channelId, {
        common: {
          name: ((_c = pager.alias) == null ? void 0 : _c.trim()) || ((_d = pager.identifier) == null ? void 0 : _d.trim()) || `Pager ${i + 1}`
        },
        native: {
          pagerId: pager.id
        }
  ...
  findPagerByPayload(payload) {
    ...
    return ((_a = this.currentAlarm) == null ? void 0 : _a.activePager) || null;
  }
  findResponseCodeEntryByCode(code) {
  ...
  findResponseCodeEntryByAnswer(answer) {
    const normalized = String(answer || "").trim();
    if (!normalized) {
      return null;
    }
    const list = Array.isArray(this.nativeConfig.responseCodes) ? this.nativeConfig.responseCodes : [];
    return list.find((item) => String(item.code) === normalized) || list.find((item) => {
      var _a;
      return ((_a = item.label) == null ? void 0 : _a.trim().toLowerCase()) === normalized.toLowerCase();
    }) || null;
  }
  findTriggersByStateId(stateId) {
    ...
    return triggers.map((trigger, index) => ({
      ...
      var _a;
      return item.trigger.enabled && ((_a = item.trigger.stateId) == null ? void 0 : _a.trim()) === stateId;
    });
  }
  clearPagerTimeout() {
  ...
  async triggerConfiguredOutput(responseEntry) {
    var _a;
    if (!responseEntry.setOutput) {
      return;
    }
    const stateId = String((_a = responseEntry.outputStateId) != null ? _a : "").trim();
    if (!stateId) {
      this.log.warn(`Antwortcode "${responseEntry.label}" hat keine Ziel-State-ID f\xFCr Folgeaktion`);
      return;
    }
    const parsedValue = this.parseOutputValue(responseEntry.outputValue);
    await this.setForeignStateAsync(stateId, parsedValue);
    this.log.info(
      `Folgeaktion ausgel\xF6st: ${stateId} = ${JSON.stringify(parsedValue)} durch Antwortcode "${responseEntry.label}"`
    );
    ...
    this.log.info(
      `Folgeaktion ${stateId} wird in ${resetDelaySec} Sekunden auf ${JSON.stringify(resetValue)} zur\xFCckgesetzt`
    );
  }
  async resetConfiguredOutput(stateId, resetValue) {
    this.clearOutputResetTimer(stateId);
    await this.setForeignStateAsync(stateId, resetValue);
    this.log.info(`Folgeaktion zur\xFCckgesetzt: ${stateId} = ${JSON.stringify(resetValue)}`);
  }
  startPagerTimeout() {
    ...
    this.log.info(`Starte Antwort-Timeout f\xFCr ${pagerLabel}: ${timeoutSec} Sekunden`);
  ...
  async pollCurrentAlarmStatus() {
    var _a;
    if (!((_a = this.currentAlarm) == null ? void 0 : _a.trackingId)) {
      return;
    }
    if (!this.currentAlarm.activePager || !this.serviceSupportsDirectResponse(this.currentAlarm.activePager.service)) {
      return;
    }
    try {
  ...
  async processRecipientStatus(recipient) {
    ...
    for (const entry of statusList) {
      ...
      if (!responseEntry && answerNoRaw !== void 0 && answerNoRaw !== null && !Number.isNaN(answerNo)) {
      ...
      this.log.info(
        `R\xFCckmeldung aus Statusabfrage gefunden: Identifier=${identifier}, answer=${answerText}, answerNo=${String(answerNoRaw != null ? answerNoRaw : "")}`
      );
      await this.handleResponseCode({
        code: responseEntry.code,
        pagerIdentifier: identifier || void 0
      });
      if (!this.currentAlarm) {
        return;
      }
  ...
  async handlePagerTimeout(expectedPagerId) {
    if (!this.currentAlarm || !this.currentAlarm.activePager) {
      return;
    }
    ...
    await this.setStateAsync("alarm.summary", `Keine Antwort von ${pagerLabel}, n\xE4chster Pager wird gepr\xFCft`, true);
    try {
  ...
  async testCredentials() {
    this.log.info("Pr\xFCfe e*Message Zugangsdaten ...");
    await this.setConnectionStatus("checking", "");
    await this.setServiceConnection(false);
    try {
      const client = this.getEMessageClient();
      await client.login();
      this.log.info("e*Message Zugangsdaten sind g\xFCltig");
  ...
  async sendPagerMessage(messageText, recipients) {
    if (!messageText.trim()) {
      throw new Error("Nachricht ist leer");
    }
    if (!recipients.length) {
      throw new Error("Es wurden keine Empf\xE4nger \xFCbergeben");
    }
    this.log.info(`Sende Nachricht an ${recipients.length} Empf\xE4nger ...`);
    const client = this.getEMessageClient();
  ...
  async sendConfiguredTestMessage() {
    ...
    const identifier = (_a = this.nativeConfig.testRecipientIdentifier) == null ? void 0 : _a.trim();
    const messageText = (_b = this.nativeConfig.testMessage) == null ? void 0 : _b.trim();
    ...
    if (this.serviceSupportsDirectResponse(serviceName) && result.trackingId) {
      ...
      await this.setStateAsync(
        "alarm.summary",
        `Testversand gesendet, warte auf R\xFCckmeldung von ${identifier}`,
        true
      );
      ...
      await this.setStateAsync(
        "alarm.summary",
        `Testversand gesendet, f\xFCr ${serviceName} wird keine direkte Pager-R\xFCckmeldung erwartet`,
        true
      );
    }
    return result;
  }
  async initializeTriggerStates() {
    var _a;
    this.triggerActiveStates = {};
    const triggers = Array.isArray(this.nativeConfig.triggerStates) ? this.nativeConfig.triggerStates : [];
    for (let index = 0; index < triggers.length; index++) {
      const trigger = triggers[index];
      const stateId = (_a = trigger.stateId) == null ? void 0 : _a.trim();
      ...
      try {
        ...
        this.log.debug(
          `Trigger initialisiert: key=${triggerKey}, state=${stateId}, aktiv=${isActive}, value=${JSON.stringify(state == null ? void 0 : state.val)}`
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        this.triggerActiveStates[triggerKey] = false;
        this.log.warn(`Konnte Trigger-Status f\xFCr ${stateId} nicht initialisieren: ${message}`);
      }
    }
  }
  async subscribeConfiguredTriggerStates() {
    var _a;
    const triggers = Array.isArray(this.nativeConfig.triggerStates) ? this.nativeConfig.triggerStates : [];
  ...
  evaluateTriggerCondition(trigger, state) {
  ...
  async sendToPager(pager, messageText) {
  ...
  async enqueueAlarm(trigger, triggerKey, state) {
    var _a;
    const messageText = (_a = trigger.messageText) == null ? void 0 : _a.trim();
    if (!messageText) {
    ...
    if (!activePagers.length) {
      this.log.warn("Ausl\xF6sung erkannt, aber es sind keine aktiven Pager konfiguriert");
      await this.setStateAsync("alarm.summary", "Ausl\xF6sung erkannt, aber keine aktiven Pager vorhanden", true);
      return;
    }
    const queueEntry = {
    ...
    this.log.info(
      `Alarm in Queue aufgenommen: ${trigger.stateId}, triggerKey=${triggerKey}, Queue-L\xE4nge=${this.alarmQueue.length}`
    );
    await this.setStateAsync(
      "alarm.summary",
      `Alarm von ${trigger.stateId} zur Queue hinzugef\xFCgt (${this.alarmQueue.length} wartend)`,
      true
    );
  ...
  async startAlarmSession(queueEntry) {
    if (this.currentAlarm) {
    ...
    await this.setStateAsync(
      "alarm.summary",
      `Alarm aktiv: ${this.formatPagerLabel(firstPager)} wurde ausgel\xF6st`,
      true
    );
    try {
      ...
      if (this.serviceSupportsDirectResponse(firstPager.service)) {
        await this.setStateAsync(
          "alarm.summary",
          `Alarm gesendet an ${this.formatPagerLabel(firstPager)}, warte auf R\xFCckmeldung`,
          true
        );
        this.startStatusPolling();
      } else {
        await this.setStateAsync(
          "alarm.summary",
          `Alarm an ${this.formatPagerLabel(firstPager)} gesendet, keine direkte Pager-R\xFCckmeldung erwartet`,
          true
        );
      }
      this.startPagerTimeout();
    ...
    this.log.debug(`Ausl\xF6sender State-Wert aus Queue: ${JSON.stringify(queueEntry.triggerValue)}`);
  }
  async finishAlarm(summaryText, acknowledged) {
    if (!this.currentAlarm) {
      return;
    }
    this.clearPagerTimeout();
  ...
  async triggerNextPager(reason) {
    ...
    await this.setStateAsync(
      "alarm.summary",
      `N\xE4chster Pager wird ausgel\xF6st: ${this.formatPagerLabel(nextPager)} (${reason})`,
      true
    );
    ...
    if (this.serviceSupportsDirectResponse(nextPager.service)) {
      await this.setStateAsync(
        "alarm.summary",
        `Alarm weitergeleitet an ${this.formatPagerLabel(nextPager)}, warte auf R\xFCckmeldung`,
        true
      );
      this.startStatusPolling();
    } else {
      await this.setStateAsync(
        "alarm.summary",
        `Alarm weitergeleitet an ${this.formatPagerLabel(nextPager)}, keine direkte Pager-R\xFCckmeldung erwartet`,
        true
      );
    }
    this.log.info(`N\xE4chster Pager ausgel\xF6st: ${this.formatPagerLabel(nextPager)}`);
    this.startPagerTimeout();
  }
  async handleResponseCode(payload) {
    var _a;
    if (!this.currentAlarm || !this.currentAlarm.activePager) {
      throw new Error("Kein aktiver Alarm vorhanden");
    }
    ...
    if (responseEntry.writeAckValue) {
      const ackValue = Number((_a = responseEntry.ackValue) != null ? _a : 0);
  ...
  async onReady() {
    ...
    try {
      ...
      const hasCredentials = !!((_a = this.nativeConfig.apiUserId) == null ? void 0 : _a.trim()) && !!((_b = this.nativeConfig.apiPassword) == null ? void 0 : _b.trim());
      if (hasCredentials) {
        try {
  ...
  async onStateChange(id, state) {
    ...
    if (id === `${this.namespace}.alarm.injectResponseCode` || id === `${this.namespace}.alarm.externalResponseCode`) {
      if (state.ack) {
        return;
      }
      ...
      this.log.info(`${stateName} \xFCber State empfangen: ${code}`);
      if (!this.currentAlarm) {
        this.log.warn(`${stateName} ignoriert: Kein aktiver Alarm vorhanden`);
        await this.setStateAsync("alarm.summary", `Kein aktiver Alarm f\xFCr ${stateName} vorhanden`, true);
        if (id === `${this.namespace}.alarm.injectResponseCode`) {
          await this.setStateAsync("alarm.injectResponseCode", 0, true);
        } else {
          await this.setStateAsync("alarm.externalResponseCode", 0, true);
    ...
    for (const item of matchingTriggers) {
      ...
      this.log.info(
        `Trigger-State \xC4nderung erkannt: ${id}, triggerKey=${triggerKey}, vorherAktiv=${wasPreviouslyActive}, jetztAktiv=${isCurrentlyActive}`
      );
      if (!isCurrentlyActive) {
        if (wasPreviouslyActive) {
          this.log.info(`Trigger ${id} (${triggerKey}) wurde zur\xFCckgesetzt und ist wieder scharf`);
        }
        this.triggerActiveStates[triggerKey] = false;
        continue;
      }
  ...
  onUnload(callback) {
    ...
    this.clearQueueProcessingTimer();
    Promise.all([this.setServiceConnection(false), this.setConnectionStatus("idle", "")]).then(() => callback()).catch(() => callback());
  }
  async onMessage(obj) {
    if (!obj || !obj.command) {
      return;
    ...
    try {
      switch (obj.command) {
        ...
        case "processResponseCode": {
          ...
          if (payload.code === void 0 || payload.code === null || Number.isNaN(Number(payload.code))) {
